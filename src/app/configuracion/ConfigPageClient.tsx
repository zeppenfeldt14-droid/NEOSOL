...
import { Upload, Link2, Trash2, Save, Image as ImageIcon, Users, Settings } from 'lucide-react'
...
export function ConfigPageClient({ currentLogo }: Props) {
  const [activeTab, setActiveTab] = useState<'visual' | 'usuarios'>('visual')
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Tab Selector - N1 Admin only */}
      {userNivel === 1 && (
        <div className="flex border-b border-white/5 pb-1">
          <button
            onClick={() => setActiveTab('visual')}
            className={`px-6 py-2.5 font-bold text-sm tracking-wider flex items-center gap-2 border-b-2 transition-all ${
              activeTab === 'visual'
                ? 'border-primary text-white bg-white/5'
                : 'border-transparent text-secondary hover:text-white'
            }`}
          >
            <Settings size={16} /> Personalización Visual
          </button>
          <button
          ...
          >
            <Users size={16} /> Control de Vendedores y Usuarios
          </button>
        </div>
      )}
